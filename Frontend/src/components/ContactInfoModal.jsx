  ...
  const fetchChatMedia = async () => {
    try {
      ...
      const response = await axios.get(
        getApiUrl(`/api/message/media/${contact._id}`),
        {
  ...
  const handleMuteToggle = async () => {
    try {
      ...
      const response = await axios.put(
        getApiUrl(`/api/message/mute/${contact._id}`),
        { mutedUntil: null },
        {
          withCredentials: true,
          headers: {
      ...
      toast.success(response.data.isMuted ? "Chat muted" : "Chat unmuted");
    } catch (error) {
  ...
  const handleBlockToggle = async () => {
    try {
      ...
      if (isBlocked) {
        ...
        // Block
        const confirmBlock = window.confirm(`Are you sure you want to block ${contact.fullname || contact.name}?`);
        if (!confirmBlock) return;
        
        await axios.post(
          getApiUrl("/api/user/block"),
          { blockedUserId: contact._id },
          {
            withCredentials: true,
            headers: {
